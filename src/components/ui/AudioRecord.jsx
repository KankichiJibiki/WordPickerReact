...
const AudioRecord = () => {
    const { startTimer, stopTimer, resetTimer, formatTime, displayTime } =
        TimerUtils();
        ...
        <>
            {isRecording ? (
                <div className="flex">
                    <button
                        type="button"
                        className="bg-blue-950 py-2 px-3 rounded-full flex mr-2 hover:bg-blue-900"
                        onClick={handleFinishRecording}
                    >
                        <FaCircleStop
                            className={`${stopButtonColor} mt-0.5 mr-3 text-xl ${stopButtonColor}`}
                        />
                        <span className="text-white">{displayTime}</span>
                    </button>
                    <button
                        type="button"
                        className="bg-blue-950 py-2 px-3 rounded-full hover:bg-blue-900"
                ...
                <button
                    type="button"
                    className="bg-blue-500 py-2 px-3 rounded-md focus:outline-none focus:shadow-outline text-white flex hover:bg-blue-400 mb-3"
            ...
            {isPlayable && !isRecording ? (
                <>
                    <h3 className="text-gray-600 text-xs border-b-2 border-gray-200 mb-1 font-bold">
                        聞いてみる
                    </h3>
                    <button
                        type="button"
                        className="bg-cyan-500 py-2 px-3 rounded-full hover:bg-cyan-400 flex text-white text-md mb-0.5"
                        onClick={playAudio}
                    >
